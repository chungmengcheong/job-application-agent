...
Never commit `.env`, user resumes, or generated `temp/` data. The backend autoloads credentials such as `OPENAI_API_KEY`, `LANGSMITH_API_KEY`, `GOOGLE_WEB_CLIENT_ID`, `ALLOWED_EMAILS`, `ALLOWED_DOMAINS`, and `CHROME_EXTENSION_ID`; ensure these exist locally before running `uvicorn`. When building Chrome packages, double-check `BACKEND_URL` so demo builds do not leak private staging endpoints, and reset any test resumes placed in `user/` after validating features.
